...
class UdfFunction:
    @staticmethod
    def sortF(book_list, rate_list):
        """
        sort by time and return the corresponding books sequence
        eg:
            input: book_list:[1,2,3]
                   rate_list:[7,10,6]
            return [3,1,2]
        """
        pairs = []
        for m, t in zip(book_list, rate_list):
            pairs.append((m, t))
        # sort by rate
        pairs = sorted(pairs, key=lambda x: x[1])
        return [x[0] for x in pairs]

# processItemSequence 处理Item的序列，将其转换为序列数据
# 取6分以上的图书序列，并且按照评分的大小进行排序
def processItemSequence(spark, rawSampleDataPath):
    ...
    userSeq = ratingSamples \
        .where(F.col("Book-Rating") >= 6) \
        .groupBy("User-ID") \
        .agg(sortUdf(F.collect_list("ISBN"), F.collect_list("Book-Rating")).alias('ISBN')) \
        .withColumn("ISBNStr", array_join(F.col("ISBN"), " "))
    userSeq.select("User-ID", "ISBNStr").show(1000, truncate = False)
    return userSeq.select('ISBNStr').rdd.map(lambda x: x[0].split(' '))
...
def trainItem2vec(spark, samples, embLength, embOutputPath, saveToRedis, redisKeyPrefix):
    ...
    synonyms = model.findSynonyms("034545104X", 20)
    ...
    with open(embOutputPath, 'w') as f:
        for ISBN in model.getVectors():
            vectors = " ".join([str(emb) for emb in model.getVectors()[ISBN]])
            f.write(ISBN + ":" + vectors + "\n")
...
if __name__ == '__main__':
    ...
    file_path = '../../resources'
    rawSampleDataPath = file_path + "/BX-Book-Ratings.csv"
    embLength = 10
    samples = processItemSequence(spark, rawSampleDataPath)
    model = trainItem2vec(spark, samples, embLength,
                          embOutputPath=file_path + "/item2vecEmb.csv", saveToRedis=False,
